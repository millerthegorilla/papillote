...
# updated for papillote by J.Miller
# ---------------------------------------------------------

echo "This script will create the Papillote ISO from files in current directory."
if [[ ${1} != "-f" ]]; then
...
mkisofs -o ../Papillote.iso -l -J -joliet-long -R -D -A "Kiosk" \
...
if [ $? -eq 0 ]; then
    echo "Saved as [1;33m`pwd | rev | cut -d/ -f2- | rev`/Papillote.iso[0m."
else
